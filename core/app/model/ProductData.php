...
class ProductData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (barcode,name,price_in,price_out,user_id,presentation,unit,category_id,inventary_min,created_at) ";
		$sql .= "value (\"$this->barcode\",\"$this->name\",\"$this->price_in\",\"$this->price_out\",$this->user_id,\"$this->presentation\",\"$this->unit\",$this->category_id,$this->inventary_min,NOW())";
...
// partiendo de que ya tenemos creado un objecto ProductData previamente utilizamos el contexto
	...
	public static function codigoExistente($codigo){
		$sql = "select * from ".self::$tablename." where barcode=\"$codigo\"";
		$query = Executor::doit($sql);
		return Model::one($query[0],new ProductData());
