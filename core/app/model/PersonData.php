...
// partiendo de que ya tenemos creado un objecto PersonData previamente utilizamos el contexto
	...
	public static function getAllClientsByPage($start_from,$limit){
		$sql = "select * from ".self::$tablename." where id>=$start_from and kind=1 limit $limit";
		$query = Executor::doit($sql);
		return Model::many($query[0],new PersonData());
	}

	public static function getAllProvidersByPage($start_from,$limit){
		$sql = "select * from ".self::$tablename." where id>=$start_from and kind=2 limit $limit";
		$query = Executor::doit($sql);
		return Model::many($query[0],new PersonData());
	}
