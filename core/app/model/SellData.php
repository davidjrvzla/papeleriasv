...
class SellData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (total,user_id,created_at) ";
		$sql .= "value ($this->total,$this->user_id,$this->created_at)";
	...
	public function add_with_client(){
		$sql = "insert into ".self::$tablename." (total,person_id,user_id,created_at) ";
		$sql .= "value ($this->total,$this->person_id,$this->user_id,$this->created_at)";
		return Executor::doit($sql);
	}
	...
	public static function getSells(){
		$sql = "select * from ".self::$tablename." where operation_type_id=2 order by created_at asc";
		//echo "sql getSells: $sql<br>";
		$query = Executor::doit($sql);
	...
	public static function getRes(){
		$sql = "select * from ".self::$tablename." where operation_type_id=1 order by created_at asc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new SellData());
	}
	...
	public static function getAllSellsByPage($start_from,$limit){
		$sql = "select * from ".self::$tablename." where id>=$start_from and operation_type_id=2 order by created_at asc limit $limit ";
		//print_r("sql: ".$sql."<br>");
	...
	public static function getAllResByPage($start_from,$limit){
		$sql = "select * from ".self::$tablename." where id>=$start_from and operation_type_id=1 order by created_at asc limit $limit";
		$query = Executor::doit($sql);
		return Model::many($query[0],new SellData());
	}
